...
import { genClientsTableColumns } from './clients-table.utils'
...
export const ClientsTable: React.FC<Props> = props => {
  const { data, pagination, loading } = props
  const { onEdit, onRemove, onChangePage, onChangePageSize } = props

  const columns: ColumnsType<ClientsTableRow> = React.useMemo(
    () => genClientsTableColumns({ onEdit, onRemove }),
    [onEdit, onRemove]
  )
